...
namespace Application.Feature.Content.Commands.UpdateContent
{
    public class UpdateContentCommandValidator : AbstractValidator<UpdateContentCommand>
    {
        ...
        private const long MaxMediaBytes = 500 * 1024 * 1024;
        ...
        public UpdateContentCommandValidator()
        {
            ...
            When(x => x.MediaFile != null && x.Type == ContentType.Video, () =>
            {
            ...
            When(x => x.MediaFile != null && x.Type == ContentType.Audio, () =>
            {
                RuleFor(x => x.MediaFile)
                    .Must(HasExtension(AudioExtensions))
                    .WithMessage("Audio must be in mp3 or wav format.")
                    .Must(f => f!.Length <= MaxMediaBytes)
                    .WithMessage("Audio size must not exceed 500MB.");
            });

            When(x => x.MediaFile != null && x.Type == ContentType.Image, () =>
            {
                RuleFor(x => x.MediaFile)
                    .Must(HasExtension(ImageExtensions))
